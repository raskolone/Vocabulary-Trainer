  ...
  return (
    <div className="flex flex-col md:flex-row gap-8">
      <div className="flex-1 space-y-4">
        <h4 className="text-sm font-bold text-content-muted mb-4 uppercase tracking-wider">{i18n.t("Połącz w pary")}</h4>
        {leftItems.map(left => {
          const matchedRightId = matches[left.id];
          const matchedRight = rightItems.find(r => r.id === matchedRightId);
          
          return (
            <motion.div layout key={left.id} className="flex items-center gap-4">
              ...
              >
                {matchedRight ? (
                  ...
                  <div className="text-content-muted/50 text-center text-sm font-medium">{i18n.t("Przeciągnij tutaj")}</div>
                )}
              </div>
            </motion.div>
          );
      ...
      <div 
        className="md:w-1/3 min-h-[200px] p-6 rounded-2xl bg-base-200/50 border border-white/10 flex flex-col"
        onDragOver={allowDrop}
        onDrop={handleDropOnPool}
      >
        <h4 className="text-sm font-bold text-content-muted mb-4 uppercase tracking-wider">{i18n.t("Dostępne opcje")}</h4>
        <div className="flex flex-col gap-3 flex-1">
          ...
          {availableRightItems.length === 0 && (
            <div className="text-center text-content-muted text-sm italic py-8">
                                        ...
                                        {i18n.t("Wszystkie opcje wykorzystane.")}
                                      </div>
          )}
        </div>
      </div>
